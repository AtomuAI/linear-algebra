...
pub struct Rot3Y<T>( pub(crate) Matrix3x3<T> )
where
...
impl<T> From<Rot3<T>> for Rot3Y<T>
where
    T: Num + 'static + Default + Copy + Debug + Neg<Output = T> + Num + Float
{
...
impl<T> Mul<Vector3<T>> for Rot3Y<T>
where
...
impl<T> TransposeAssignTo for Rot3Y<T>
where
    T: Num + 'static + Default + Copy + Debug + Neg<Output = T> + AddAssign + Num + Float
{
    type Output = Self;
