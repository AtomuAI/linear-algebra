...
pub struct Quaternion<T>( Vector4<T> )
where
    ...
    T: 'static + Default + Copy + Debug + Neg<Output = T> + Num + Float
{
    ...
    pub fn x( &self ) -> &T { &self[ 1 ] }
    pub fn y( &self ) -> &T { &self[ 2 ] }
    pub fn z( &self ) -> &T { &self[ 3 ] }
}
...
mod tests {
    ...
    */
    ...
    fn quat_conj_test() {
        ...
        println!( "{:?}", conj );
        assert_eq!( conj[ 0 ], 1.0 );
        assert_eq!( conj[ 1 ], 2.0 );
        assert_eq!( conj[ 2 ], 3.0 );
        assert_eq!( conj[ 3 ], 4.0 );
    }
}
